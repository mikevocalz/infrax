...
const STRAIGHT_TALK: { title: string; icon: LucideIcon }[] = [
  { title: 'No Guessing', icon: Target },
  { title: 'No Noise', icon: Search },
  { title: 'No Friction', icon: ShieldCheck },
];

export default function WhyInfraXPage() {
  return (
      ...
      <section className="relative h-[60vh] flex items-center bg-navy overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 pt-20">
          <div className="max-w-4xl space-y-6">
            <div className="section-label !text-orange-accent">The InfraX Difference</div>
            <h1 className="text-5xl md:text-[8rem] font-black text-white uppercase tracking-tighter leading-none">
              STRATEGY <br />
              <span className="text-orange-accent">FIRST.</span>
            </h1>
          </div>
        </div>
      </section>
          ...
          <div className="relative p-16 bg-navy text-white space-y-8 group overflow-hidden">
            ...
            <div className="relative z-10 space-y-8">
              ...
              <p className="text-white/60 font-light text-lg">
                Most infrastructure companies are incentivized to push specific inventory. We don&apos;t take placement
                fees that bias our advice.
              </p>
              <ul className="space-y-4">
                {['No Placement Fees', 'Zero Inventory Bias', 'Ruthless Filtering'].map((item) => (
                  <li key={item} className="flex items-center gap-3 text-xs font-bold uppercase tracking-widest text-orange-accent">
                    <div className="w-2 h-2 bg-orange-accent" /> {item}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="relative p-16 bg-white space-y-8 group overflow-hidden">
            ...
            <div className="relative z-10 space-y-8">
              ...
              <p className="text-slate-500 font-light text-lg">
                Our job doesn&apos;t end with a shortlist. We manage risk, timelines, and accountability from matching
        ...
        <section className="bg-navy p-12 lg:p-32 text-white text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="max-w-4xl mx-auto space-y-12 relative z-10">
            <h2 className="text-5xl lg:text-8xl font-black uppercase tracking-tighter leading-none">
              Straight Talk. <br />
              <span className="text-orange-accent">Serious Results.</span>
            </h2>
            <div className="grid md:grid-cols-3 gap-12 text-left pt-12">
              {STRAIGHT_TALK.map(({ title, icon: Icon }) => (
                <div key={title} className="space-y-4 p-8 bg-white/5 border border-white/10">
                  <div className="w-12 h-12 flex items-center justify-center bg-white/5 border border-white/10">
                    <Icon className="w-6 h-6 text-orange-accent" />
                  </div>
                  <div className="text-2xl font-black uppercase tracking-tight">{title}</div>
                </div>
              ))}
            </div>
          </div>
