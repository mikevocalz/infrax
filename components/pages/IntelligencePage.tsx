...
import { ArrowUpRight, Clock, ShieldCheck, Database, Filter } from 'lucide-react';
import type { MarketSignal } from '@/types';

const FILTERS = ['All', 'GRID', 'POLICY', 'CAPACITY'];
  ...
  useEffect(() => {
    const fetchSignals = async () => {
      try {
        ...
        const data: MarketSignal[] = await response.json();
        setSignals(data);
      } catch (error) {
      ...
      <section className="container mx-auto px-6 mb-16 md:mb-24">
        <div className="max-w-4xl space-y-6 md:space-y-8">
          ...
          <p className="text-lg md:text-2xl text-slate-500 font-light leading-relaxed max-w-2xl">
            InfraX aggregates and curates infrastructure intelligence across utility filings, interconnection queues,
            and operator disclosures.
          </p>
        </div>
      </section>
            ...
            {isLoading ? (
              ...
              <div className="grid gap-6">
                {filtered.map((signal, i) => (
                  <div
                    key={signal.id ?? i}
                    className="bg-white border border-slate-200 p-8 group hover:border-orange-accent transition-all hover:shadow-xl relative overflow-hidden"
                  >
                    <div
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                      <div className="space-y-4 flex-1">
                        <div className="flex items-center gap-4">
                          <span
                            className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest text-white ${
                              signal.severity === 'High'
                                ? 'bg-red-500'
                                : signal.severity === 'Medium'
                                ? 'bg-orange-accent'
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex justify-between items-center border-b border-slate-200 pb-3">
                      ...
                      <div className="text-right">
                        <div
                          className={`text-xs font-black ${item.status === 'Critical' ? 'text-red-500' : 'text-navy'}`}
                        >
                          {item.status}
                        </div>
