      ...
      <div className="container mx-auto px-6 py-24 lg:py-40 space-y-32">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
          {LEADERSHIP.map((leader, i) => (
            <div
              key={i}
              className="bg-slate-50 p-8 lg:p-16 border border-navy/5 flex flex-col group transition-all hover:shadow-2xl hover:-translate-y-1"
            >
              <div className="flex flex-col sm:flex-row sm:items-center gap-6 mb-10">
                ...
                <div>
                  <h3 className="text-3xl font-black text-navy uppercase tracking-tight leading-none mb-2">
                    {leader.name}
                  </h3>
                  <div className="text-[10px] font-bold text-orange-accent uppercase tracking-[0.2em]">
                    {leader.role}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-10">
                {leader.expertise.map((exp) => (
                  <span key={exp} className="px-3 py-1.5 bg-navy text-white text-[9px] font-bold uppercase tracking-widest">
                    {exp}
                  </span>
                ))}
              </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center relative z-10">
            <div className="space-y-8">
              ...
              <p className="text-lg sm:text-xl text-slate-500 font-light leading-relaxed max-w-xl">
                InfraX is built on truth, not inventory. We combine proprietary intelligence with relationships to
                solve the fragmentation of the global grid.
              </p>
            </div>
